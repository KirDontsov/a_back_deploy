...
#[actix_web::get("/avito/ad_field_values/{id}")]
pub async fn get_avito_ad_field_value_by_id(
	user: JwtMiddleware,
	...
	{
		Ok(avito_ad_field_value) => avito_ad_field_value,
			...
			.first::<crate::models::AvitoAdField>(&mut conn)
		{
			...
			Err(diesel::result::Error::NotFound) => {
				return Ok(HttpResponse::Forbidden().json(json!({
					"status": "fail",
					"message": "You don't have permission to access this ad field value"
				})));
			}
			Err(_) => {
				return Ok(HttpResponse::InternalServerError().json(json!({
