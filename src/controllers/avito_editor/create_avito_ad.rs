...
pub async fn create_avito_ad(
	...
	let existing_feed: Option<crate::models::AvitoFeed> =
	...
	let avito_ad_id = parsed.get("avito_ad_id").and_then(|v| v.as_str()).map(|s| s.to_string());
	let parsed_id = parsed.get("parsed_id").and_then(|v| v.as_str()).map(|s| s.to_string());
				...
				Ok(avito_ad) => {
					...
					if let Some(fields) = &fields {
						for field_data in fields {
							// Create the ad field
						let new_field_result = diesel::insert_into(crate::schema::avito_ad_fields::table)
							.values((
								...
								crate::schema::avito_ad_fields::field_type.eq(&field_data.field_type),
