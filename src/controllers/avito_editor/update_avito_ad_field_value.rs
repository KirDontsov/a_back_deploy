...
use crate::{
	models::{AvitoAdFieldValue, UpdateAvitoAdFieldValue},
	AppState,
};
...
#[actix_web::patch("/avito/ad_field_values/{id}")]
pub async fn update_avito_ad_field_value(
	user: JwtMiddleware,
	path: web::Path<String>,
	body: web::Json<UpdateAvitoAdFieldValue>,
			...
			.first::<crate::models::AvitoAdField>(&mut conn)
		{
			...
			Err(diesel::result::Error::NotFound) => {
				return Ok(HttpResponse::Forbidden().json(json!({
					"status": "fail",
					"message": "You don't have permission to update this ad field value"
				})));
			}
			Err(_) => {
				return Ok(HttpResponse::InternalServerError().json(json!({
	...
	// Update the ad field value
	let updated_avito_ad_field_value =
		diesel::update(crate::schema::avito_ad_field_values::table.find(field_value_id))
			.set((body
				.value
				.as_ref()
				.map(|v| crate::schema::avito_ad_field_values::value.eq(v)),))
			.get_result::<AvitoAdFieldValue>(&mut conn);

	match updated_avito_ad_field_value {
		...
		Err(_) => Ok(HttpResponse::InternalServerError().json(json!({
			"status": "error",
			"message": "Failed to update avito ad field value"
		}))),
	}
}
