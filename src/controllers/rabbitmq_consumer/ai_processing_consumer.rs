...
pub async fn start_ai_processing_consumer(ws_server: WebSocketConnections) {
	let rabbitmq_url =
			...
			Ok(connection) => {
				match connection.create_channel().await {
					Ok(channel) => {
						...
						log::info!(
							"Successfully bound queue {} to exchange {} with routing key result.*",
							queue.name(),
							"avito_exchange"
						);
							...
							.await
						{
							Ok(consumer) => {
								...
								log::info!(
									"Started consuming from queue: {}, for AI processing results",
									queue.name().as_str()
								);
								consumer
							}
							...
							match delivery_result {
								Ok(delivery) => {
									...
									match std::str::from_utf8(&delivery.data) {
										Ok(json_str) => {
											log::info!(
												"Received AI processing result JSON string: {}",
												json_str
											);
								...
								Err(e) => {
									eprintln!(
										"Error receiving AI processing result message: {:?}",
										e
									);
									log::error!(
										"Error receiving AI processing result message: {:?}",
					...
					Err(e) => {
						eprintln!(
							"Failed to create RabbitMQ channel for AI processing consumer: {:?}",
							e
						);
...
fn extract_user_id(json_value: &Value) -> Option<uuid::Uuid> {
	...
	} else if let Some(user_id_val) = json_value
		.get("userId")
...
fn extract_task_id(json_value: &Value) -> Option<uuid::Uuid> {
	...
	} else if let Some(task_id_val) = json_value
		.get("taskId")
		.or_else(|| json_value.get("TASK_ID"))
	{
		// Also check for alternative field names
		task_id_val
			.as_str()
