    ...
    match ads_result {
        Ok(ads) => {
            ...
            writer
                ...
                .map_err(|e| actix_web::error::ErrorInternalServerError(format!("CSV write error: {:?}", e)))?;
            ...
            for ad in &ads {
                writer
                    .write_record(&[
                        ...
                        ad.run_date.as_ref().map(|dt| dt.to_rfc3339()).unwrap_or_else(|| "".to_string()).as_str(),
                        ad.city_query.as_deref().unwrap_or(""),
                        ad.search_query.as_deref().unwrap_or(""),
                        ad.position.map(|p| p.to_string()).unwrap_or(String::new()).as_str(),
            ...
            let csv_bytes = writer.into_inner()
                .map_err(|e| actix_web::error::ErrorInternalServerError(format!("CSV finish error: {:?}", e)))?
                .into_inner();

            // Generate filename using search_query and date
            let filename = if !ads.is_empty() {
                let first_ad = &ads[0];
                let search_query = first_ad.search_query.clone().unwrap_or_else(|| "ads".to_string());
